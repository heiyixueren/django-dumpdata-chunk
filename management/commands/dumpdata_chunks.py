...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--output-folder', default='.', dest='output_folder',
                            help='Specifies the output folder where the data should be dumped, e.g. ./dumped-data. '
                                 'Please make sure this folder exists.')
        parser.add_argument('--max-records-per-chunk', default=100000, dest='max_records_per_chunk', type=int,
                            help='Specifies the number of records per each .json dumpfile. '
                                 'By default --max-records-per-chunk=100000.')
            ...
            if not model._meta.proxy and router.allow_migrate_model(using, model):
                if use_base_manager:
                    ...
                    for chunk_num in range(0, chunks_total):
                        output_objects = model._default_manager.using(using).all().order_by(model._meta.pk.name)[
                                         chunk_num * max_records_per_chunk:(chunk_num + 1) * max_records_per_chunk]
                        if output_objects:
                            chunk_count += 1
